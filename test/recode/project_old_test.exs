defmodule Recode.ProjectOldTest do
  use ExUnit.Case

  alias Recode.ModuleInfo
  alias Recode.ProjectOld

  describe "new/1" do
    test "creates a project map form a simple module" do
      assert ProjectOld.new(["test/fixtures/traverse/simple.ex"]) ==
               %{
    ...
    test "creates a project map from rename/lib/definition.ex" do
      assert ProjectOld.new(["test/fixtures/rename/lib/definition.ex"]) ==
               %{
    ...
    test "creates a project map from rename/lib/import.ex" do
      assert ProjectOld.new(["test/fixtures/rename/lib/import.ex"]) ==
               %{
                 [:Rename, :Bar] => %ModuleInfo{
                   aliases: [],
